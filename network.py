...
normal_sampling = torch.tensor([-1.64761461, -1.45512392, -1.33302146, -1.24118489, -1.16649474,
                                -1.10293101, -1.0472058, -0.99731721, -
                                0.95195146, -0.91019756, -0.87139658, -0.83505566,
                                -0.80079598, -0.76831968, -0.73738809, -
                                0.70780676, -0.67941502, -0.65207844, -0.62568332,
                                -0.6001325, -0.57534225, -0.55123978, -
                                0.52776141, -0.50485098, -0.4824587, -0.46054014,
                                -0.43905545, -0.41796868, -0.39724726, -
                                0.37686153, -0.35678436, -0.33699083, -0.31745792,
                                -0.29816434, -0.27909025, -0.26021714, -
                                0.24152764, -0.22300538, -0.20463492, -0.18640156,
                                -0.16829133, -0.15029084, -0.13238723, -
                                0.11456811, -0.09682146, -0.0791356, -0.06149911,
                                -0.04390081, -0.02632966, -
                                0.00877475, 0.00877475,  0.02632966,  0.04390081,  0.06149911,
...
class LinearNet(Module):
    ...
    def Sigma_dot(self, i, retain_graph=False):
        alpha_dot = torch.autograd.grad(
            self.alpha[i-1].sum(), self.pre_alpha[i-1], retain_graph=retain_graph)[0]
        return torch.matmul(alpha_dot, torch.t(alpha_dot)) / self.widths[i-1]

    def Sigma_ddot(self, i, retain_graph=False):
        alpha_dot = torch.autograd.grad(
            self.alpha[i-1].sum(), self.pre_alpha[i-1], create_graph=True)[0]
        alpha_ddot = torch.autograd.grad(
            alpha_dot.sum(), self.pre_alpha[i-1], retain_graph=retain_graph)[0]
    ...
    def moments_S(self):
        ...
        for j in range(1, self.depth):
            alpha_dot = torch.autograd.grad(
                self.alpha[j].sum(), self.pre_alpha[j], create_graph=True)[0]
            alpha_ddot = torch.autograd.grad(
                alpha_dot.sum(), self.pre_alpha[j], create_graph=True)[0]
            alpha_dddot = torch.autograd.grad(
                alpha_ddot.sum(), self.pre_alpha[j], retain_graph=True)[0]

            Sigma = self.Sigma(j+1)
            Sigma_dot = torch.matmul(
                alpha_dot, torch.t(alpha_dot)) / self.widths[j]
            Sigma_ddot = torch.matmul(
                alpha_ddot, torch.t(alpha_ddot)) / self.widths[j]
            Mixed = torch.matmul(alpha_ddot, torch.t(
                self.alpha[j])) / self.widths[j]
            Mixed_dot = torch.matmul(
                alpha_dddot, torch.t(alpha_dot)) / self.widths[j]
            ...
            move_m1 += NTK * \
                (covar_m1.diag().view([-1, 1]) *
                 Mixed_dot + covar_m1 * Sigma_ddot)
            move_m1 += covar_m1.diag().view([-1, 1]) * \
                Mixed + covar_m1 * Sigma_dot
            move_m1 += NTK.diag().view([-1, 1]) * (NTK * Mixed_dot + Mixed)

            covar_m1 = covar_m1 * Sigma_dot + \
                (covar_m1.diag() + NTK.diag()).view([-1, 1]) * Mixed
...
class Conv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.stdv = 1. / math.sqrt(in_channels*1.0)
        self.mask = make_mask(
            kernel_size[0], stride[0]).unsqueeze(-1)*make_mask(kernel_size[1], stride[1])
...
class Conv3d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.mask = self.mask.unsqueeze(-1) * \
            make_mask(kernel_size[2], stride[2])
...
class ConvTranspose1d(_ConvTransposeNd):
    ...
    def forward(self, input, output_size=None):
        ...
        return F.conv_transpose1d(
            input*self.stdv, self.weight*self.mask, self.bias *
            self.beta, self.stride, self.padding,
...
class ConvTranspose2d(_ConvTransposeNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.stdv = 1. / math.sqrt(in_channels)
        self.mask = make_mask_transpose(
            kernel_size[0], stride[0]).unsqueeze(-1)*make_mask_transpose(kernel_size[1], stride[1])
    ...
    def forward(self, input, output_size=None):
        ...
        return F.conv_transpose2d(
            input*self.stdv, self.weight*mask, self.bias *
            self.beta, self.stride, self.padding,
            self.output_padding, self.groups, self.dilation)
